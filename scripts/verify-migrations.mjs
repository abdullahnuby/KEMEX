...
const required = [
  '018_schema_recovery_breakdown_trips.sql',
  '019_domain_integrity_hardening.sql',
  '020_rbac_audit_hardening.sql',
  '021_workflow_integrity.sql',
  '022_operational_audit_history.sql',
  '023_action_authorization_and_integrity.sql',
  '024_enterprise_data_controls.sql',
  '025_driver_transport_execution.sql',
  '026_data_management.sql',
]
for (const f of required) {
  if (!files.includes(f)) throw new Error(`Missing required enterprise migration: ${f}`)
}

const sql = files.map(f => fs.readFileSync(path.join(dir, f), 'utf8')).join('\n')
for (const needle of [
  'CREATE OR REPLACE FUNCTION private.kemex_can_transition',
  ...
  if (!sql.toLowerCase().includes(needle.toLowerCase())) {
    throw new Error(`Missing required migration construct: ${needle}`)
  }
}
