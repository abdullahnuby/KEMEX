...
export function AnalyticsLineChart({ points, height = 230, valueSuffix = '' }: { points: ChartPoint[]; height?: number; valueSuffix?: string }) {
  const safe = points.length ? points : [{ label: '—', value: 0 }]
  const max = Math.max(1, ...safe.map((p) => p.value))
  const min = Math.min(0, ...safe.map((p) => p.value))
  ...
  const path = safe.map((p, i) => `${i === 0 ? 'M' : 'L'} ${x(i).toFixed(1)} ${y(p.value).toFixed(1)}`).join(' ')
  const area = `${path} L ${x(safe.length - 1).toFixed(1)} ${top + chartH} L ${x(0).toFixed(1)} ${top + chartH} Z`
  return <div className="analytics-chart" dir="ltr">
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="رسم بياني خطي">
      {[0, .25, .5, .75, 1].map((tick) => {
        const yy = top + chartH * tick
        const val = max - range * tick
        return <g key={tick}><line x1={left} x2={width - right} y1={yy} y2={yy} className="chart-grid-line"/><text x={left - 8} y={yy + 4} textAnchor="end" className="chart-axis-label">{num(val)}</text></g>
      })}
      <path d={area} className="chart-area" />
      <path d={path} className="chart-line" fill="none" />
      {safe.map((p, i) => <g key={`${p.label}-${i}`}>
        <circle cx={x(i)} cy={y(p.value)} r="4.5" className="chart-point" />
        <text x={x(i)} y={height - 11} textAnchor="middle" className="chart-x-label">{p.label}</text>
        <title>{`${p.label}: ${num(p.value)}${valueSuffix}`}</title>
      </g>)}
    </svg>
  </div>
}
...
function EmptyChart() {
  return <div className="analytics-empty"><span>لا توجد بيانات كافية للتحليل</span></div>
}
