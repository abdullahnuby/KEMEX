...
export function AlertsPage({assets,workOrders,drivers,contracts,onRoute,alertDays=30,alertKm=1500,alertHours=80,plans=[],oils=[]}:{assets:Asset[];workOrders:WorkOrder[];drivers:Driver[];contracts:Contract[];onRoute:(route:string)=>void;alertDays?:number;alertKm?:number;alertHours?:number;plans?:Record<string,unknown>[];oils?:Record<string,unknown>[]}) {
...
function buildAlerts(assets:Asset[],workOrders:WorkOrder[],drivers:Driver[],contracts:Contract[],alertDays:number,alertKm:number,alertHours:number,plans:Record<string,unknown>[],oils:Record<string,unknown>[]):Alert[] {
  const out:Alert[]=[]
  ...
  for(const p of [...plans,...oils]){
    ...
    if(cadence>0){
      ...
      if(remaining<=threshold) out.push({id:`meterdue-${String(p.id)}`,title:remaining<=0?'استحقاق عداد متجاوز':'استحقاق عداد قريب',entity:a.name || a.code,detail:remaining<=0?'يجب إنشاء/تنفيذ الإجراء المستحق.':`متبقٍ ${remaining.toLocaleString('ar-EG')} ${a.mt==='كم'?'كم':'ساعة'}`,severity:remaining<=0?'عالي':'متوسط',route:'oils',icon:'maintenance'})
    }
  }
