  ...
  return <div className="space-y-6">
    ...
    <DataTable
      ...
      columns={[
        ...
        { id:'print', header:'طباعة', hideable:false, searchable:false, render:(r:RecordType)=><PrintRecordButton documentTitle={String(r.po||'').trim()?'أمر شراء':'طلب شراء'} documentNumber={String(r.po||r.number||r.id||'')} documentDate={dateText(String(r.date??''))} documentStatus={String(r.status??'')} meta={[{label:'مقدم الطلب',value:String(r.req??'—')},{label:'المشروع',value:<ReferenceValue field="proj" value={r.proj} lookups={{projects}} />},{label:'المورد',value:String(r.supplier??'—')},{label:'رقم أمر الشراء',value:String(r.po??'—')}]} signatures={[{label:'مقدم الطلب'},{label:'مسؤول المشتريات'},{label:'الاعتماد المالي'},{label:'المورد'}]} footerNote="مستند شراء صادر من نظام KEMEX لإدارة المخازن والمشتريات."><div className="print-section-title">تفاصيل الصنف والكمية</div><table><tbody><tr><th>البند</th><th>القيمة</th></tr><tr><td>الوصف</td><td>{String(r.desc??'—')}</td></tr><tr><td>الكمية</td><td>{`${fmt(Number(r.qty||0))} ${String(r.unit??'')}`}</td></tr><tr><td>التكلفة التقديرية</td><td>{formatMoney(Number(r.est||0))}</td></tr><tr><td>المستلم حتى الآن</td><td>{`${fmt(Number(r.receivedQty||0))} / ${fmt(Number(r.qty||0))} ${String(r.unit??'')}`}</td></tr><tr><td>حالة الاستلام</td><td>{String(r.receiptStatus??'غير مستلم')}</td></tr><tr><td>ملاحظات</td><td>{String(r.notes??'—')}</td></tr></tbody></table></PrintRecordButton> },
