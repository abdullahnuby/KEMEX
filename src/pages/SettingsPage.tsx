...
type Settings={company_name:string;group_name:string;currency_code:string;vat:number;diesel:number;petrol:number;alert_days:number;alert_km:number;alert_hours:number;trip_geofence_radius_m:number}
const empty:Settings={company_name:'',group_name:'',currency_code:'EGP',vat:0,diesel:0,petrol:0,alert_days:30,alert_km:1500,alert_hours:80,trip_geofence_radius_m:1000}

/** Organization settings editor. Currency is persisted centrally and defaults to EGP. */
export function SettingsPage({user,repository,onSaved}:{user:User;repository:Repository;onSaved?:(settings:Settings)=>void}){
 const [s,setS]=useState<Settings>(empty);const [busy,setBusy]=useState(false);const [msg,setMsg]=useState('');const [loadError,setLoadError]=useState('');const [loading,setLoading]=useState(true); const [tab,setTab]=useState<'general'|'data'>('general')
 useEffect(()=>{let active=true;setLoading(true);setLoadError('');repository.getSettings().then(v=>{if(active)setS({...empty,...v,currency_code:normalizeCurrencyCode(String(v.currency_code??'EGP'))})}).catch(e=>{if(active)setLoadError(e instanceof Error?e.message:'تعذر تحميل الإعدادات. حاول تحديث الصفحة.')}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[repository])
 const can=canManageUsers(user.role)
 async function save(){if(!can||busy)return;setMsg('');const numeric:[keyof Settings,string][]=[['vat','ضريبة القيمة المضافة'],['diesel','سعر الديزل'],['petrol','سعر البنزين'],['alert_days','مدة التنبيه بالأيام'],['alert_km','مسافة التنبيه'],['alert_hours','ساعات التنبيه'],['trip_geofence_radius_m','نطاق التحقق الجغرافي للتحميل والتسليم (متر)']];for(const [key,label] of numeric){const n=s[key];if(typeof n!=='number'||!Number.isFinite(n)||n<0 || (key==='trip_geofence_radius_m' && n<1)){setMsg(`تحقق من قيمة ${label}؛ يجب أن تكون رقمًا غير سالب.`);return}}setBusy(true);try{await repository.saveSettings({...s,currency_code:normalizeCurrencyCode(s.currency_code)});onSaved?.(s);setMsg('تم حفظ الإعدادات بنجاح.')}catch(e){setMsg(e instanceof Error?e.message:'تعذر حفظ الإعدادات. حاول مرة أخرى.')}finally{setBusy(false)}}
 const update=(k:keyof Settings,v:string)=>setS({...s,[k]:['vat','diesel','petrol','alert_days','alert_km','alert_hours','trip_geofence_radius_m'].includes(k)?Number(v):k==='currency_code'?normalizeCurrencyCode(v):v})
 return (
  <div className="space-y-6">
    <div className="settings-tabbar">
      ...
      <button
        className={tab === 'data' ? 'settings-tab active' : 'settings-tab'}
        type="button"
        onClick={() => setTab('data')}
      >
    ...
    {tab === 'data' ? (
      ...
      <>
        <PageHeader
          ...
          description="بيانات المؤسسة والعملة والأسعار وقواعد التنبيه الافتراضية للنظام."
        />
        ...
        <Card>
          ...
          <div className="modal-actions">
            ...
            {can && (
              <Button icon={<Save size={15} />} disabled={busy || loading || !!loadError} loading={busy} onClick={save}>
