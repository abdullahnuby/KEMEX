...
The first non-breaking architecture slice is now implemented.

### Implemented

- `src/app/routing/routeRegistry.ts` is the canonical route metadata registry for current routes, route intent, module ownership, titles and descriptions.
- `src/app/routing/RouteGuard.tsx` centralizes route-level authorization presentation and delegates capability decisions to `src/config/app.ts`.
- `src/config/modules.ts` no longer owns a second write-permission matrix; it re-exports `canWriteModule` from the canonical permission source.
- `src/app/query/queryKeys.ts` centralizes the existing bootstrap query key so future feature queries can be introduced without another invalidation convention.
- `App.tsx` now consumes the route registry metadata and the shared guard/query-key contracts while preserving the existing route paths and page composition.

### Intentionally deferred

- Moving actual `<Route>` element declarations out of `App.tsx`.
- Splitting the global bootstrap query into feature-scoped queries.
- Moving mutation orchestration out of `App.tsx`.
- Restructuring all page folders into feature directories.

These are deferred to avoid a broad rewrite before the current application has a verified build environment and regression suite.

### Verification

Static source inspection completed successfully. Full TypeScript verification remains blocked by the supplied environment: the repository declares Node `>=24`, while the available runtime is Node `22.x`, and the installed `node_modules/@types/react` and `@types/react-dom` directories are empty in the execution environment. No dependency declaration was changed to conceal this environmental failure.

---

# Phase 1 — One-Sprint Implementation Closeout

**Date:** 2026-09-23

The Phase 1 implementation sprint completed the non-breaking architecture extraction planned above.

## Implemented in this sprint

### Routing boundary

- Route rendering moved from `src/App.tsx` to `src/app/routing/AppRoutes.tsx`.
- URL paths remain unchanged.
- The canonical route registry remains in `src/app/routing/routeRegistry.ts`.
- Static parity verification confirms **39 rendered routes / 39 registry routes** with no missing, extra, or duplicate paths.
- URL parameter resolution remains inside stable route wrapper components.

### Authorization boundary

- `RouteGuard` remains the route-level presentation boundary.
- `canViewModule` and `canWriteModule` remain owned by `src/config/app.ts`.
- `src/config/modules.ts` re-exports `canWriteModule` instead of maintaining another permission matrix.
- Navigation-specific permission aliases are represented by `permissionModule` metadata instead of hardcoded route exceptions in `Layout.tsx`.

### Query/data boundary

The previous monolithic bootstrap `Promise.allSettled` query was replaced by domain query boundaries while preserving partial-load behavior:
...
`useKemexBootstrap` is now a compatibility composition facade that preserves the page-facing data shape while domain queries own their React Query keys and loading/error states.

### Mutation boundary

`src/features/app/hooks/useKemexMutations.ts` now owns:

- repository writes
- standardized mutation error messages
- query invalidation
- logout cache clearing
- workflow orchestration for operations, assignments, and requests
- inventory purchase/reception helpers

`App.tsx` no longer contains business mutation implementations.

### Application composition

`App.tsx` is now limited to:

- auth/session composition
- route location/navigation wiring
- bootstrap composition
- audit actor synchronization
- global error presentation
- application shell composition

This is the intended application-composition role for the file.

## Verification performed

The following static checks pass:

- Route registry parity: **39/39**.
- No duplicate route definitions.
- No route declarations remain in `App.tsx`.
- `App.tsx` imports the routing and mutation boundaries.
- Domain query boundaries are present and composed by the bootstrap facade.
- Mutation invalidation is centralized at the KEMEX query namespace.
- Logout removes the KEMEX query namespace from cache.
- Required existing specialized page files remain present.
- Legacy desktop sidebar markup is not mounted by `Layout.tsx`.
- TypeScript/TSX syntax transpilation passes for the nine affected source files.
- `scripts/verify-source.mjs` was replaced with architecture-aware integrity checks and passes.

## Verification limitation

A full `npm run build` / `npm run typecheck` verification could not be completed in the provided execution environment. The repository requires Node `>=24`, while the available runtime is Node `22.16.0`; dependency installation also did not complete, leaving the local dependency tree incomplete. No dependency or application behavior was changed merely to make the environment appear green.

This is an **environment verification blocker**, not an application failure proven by the static checks above.

## Deferred from Phase 1

- Full browser/E2E regression coverage.
- Visual regression validation.
- Feature-folder relocation of every existing page.
- Removal of all remaining global CSS overlap.
- Full runtime build/typecheck/lint gate once Node 24 and a complete dependency install are available.

These items belong to subsequent verification or design-system work and were intentionally not mixed into the architecture extraction sprint.

## Phase 4 UX architecture integration

The operational UX layer now sits above existing feature pages without replacing their data or mutations. Shared workflow primitives are located under `src/shared/ui`, while domain workflow mappings live in `src/shared/workflows/workflowDefinitions.ts`.

The supported operational journeys are: vehicle lifecycle, maintenance lifecycle, and transportation lifecycle. Generic workflow-enabled list modules expose a role-aware operational queue summary while retaining their existing repository-backed CRUD behavior.
