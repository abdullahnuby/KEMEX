...
export function TripsPage({assets,drivers,projects,clients,onRoute,initialView='list',currencyCode='EGP'}:{assets:Asset[];drivers:Driver[];projects:Project[];clients:Customer[];onRoute:(path:string)=>void;initialView?:'list'|'board';currencyCode?:string}) {
 ...
 const columns:DataTableColumn<Trip>[]=[
  ...
  {id:'actions',header:'إجراء',render:t=><div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={()=>onRoute(`trips/${t.id}`)}>التفاصيل</Button>{t.status==='received'&&t.is_billable&&!t.invoice_id?<Button size="sm" onClick={()=>onRoute(`trips/${t.id}`)}>ربط الفاتورة</Button>:TRIP_STATUS_TRANSITIONS[t.status][0]&&<Button size="sm" onClick={()=>requestAdvance(t)}>{TRIP_STATUS_LABELS[TRIP_STATUS_TRANSITIONS[t.status][0]]}</Button>}</div>}
 ]
 const advance=async(t:Trip)=>{setAdvanceBusy(true);try{const next=TRIP_STATUS_TRANSITIONS[t.status][0];if(!next)return;await tripsService.updateStatus(t.id,next);await load()}catch(e){setError(e instanceof Error?e.message:'تعذر تغيير حالة النقل.')}finally{setAdvanceBusy(false)}}
 const requestAdvance=(t:Trip)=>setPendingAdvance(t)
 const confirmAdvance=async()=>{if(!pendingAdvance)return;const trip=pendingAdvance;setPendingAdvance(null);await advance(trip)}
 ...
 <ConfirmModal
  ...
  title={pendingAdvance ? `تأكيد الانتقال في ${pendingAdvance.trip_number}` : 'تأكيد الانتقال'}
  description={pendingAdvance ? <>سيتم نقل الرحلة من <strong>{TRIP_STATUS_LABELS[pendingAdvance.status]}</strong> إلى <strong>{TRIP_STATUS_LABELS[TRIP_STATUS_TRANSITIONS[pendingAdvance.status][0]]}</strong>. تأكد من اكتمال المتطلبات التشغيلية قبل المتابعة.</> : ''}
  confirmLabel={pendingAdvance ? `نقل إلى ${TRIP_STATUS_LABELS[TRIP_STATUS_TRANSITIONS[pendingAdvance.status][0]]}` : 'تأكيد'}
  busy={advanceBusy}
  onCancel={()=>!advanceBusy&&setPendingAdvance(null)}
